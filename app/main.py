...
async def back_home(query: CallbackQuery, state: FSMContext) -> None:
    ...
    if query.message is not None:
        # Se il callback nasce da un messaggio testuale, edit_text è corretto.
        # I flussi foto usano già messaggi separati nel publisher.
...
async def future_sections(query: CallbackQuery) -> None:
    names = {
        "menu:stats": "📊 Statistiche",
        "menu:settings": "⚙️ Impostazioni",
    }
    section = names.get(query.data, "Funzione")
    await query.answer(
...
async def main() -> None:
    ...
    # Tutti i model Fase 11 vengono importati tramite i router/service
    # prima di init_db(), quindi create_all crea esclusivamente le nuove tabelle.
    await init_db()
